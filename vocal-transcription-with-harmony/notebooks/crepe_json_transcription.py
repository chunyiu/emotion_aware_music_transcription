...
output_dir = "./predicted_crepe_json_folder"
frame_rate = 100  # Hz (10 ms step)
...
# === Helper: ensure unique filenames ===
...
def json_to_notes_direct(json_file):
    ...
    if not isinstance(data, list) or len(data) == 0:
        print(f"No note data in {json_file}")
        return []

    # Extract note events directly
    note_events = []
    for entry in data:
        if "note_start" in entry and "note_end" in entry and "note" in entry:
            for s, e, p in zip(entry["note_start"], entry["note_end"], entry["note"]):
                if p > 0 and e > s:  # Valid note
                    note_events.append([float(s), float(e), float(p)])
    
    if not note_events:
        print(f"No valid notes found in {json_file}")
        return []
    
    # Sort by start time
    note_events.sort(key=lambda x: x[0])
    
    # Optional: merge very close notes (within 0.1s and ±1 semitone)
    merged = [note_events[0]]
    for s, e, p in note_events[1:]:
        last_s, last_e, last_p = merged[-1]
        if abs(p - last_p) <= 1.0 and s - last_e <= 0.1:
            merged[-1][1] = e  # Extend duration
            merged[-1][2] = (p + last_p) / 2.0  # Average pitch
        else:
            merged.append([s, e, p])
    
    return merged

# === JSON → Reference Pitch Sequence (for frame-level eval) ===
...
def correct_octave_errors(freqs, max_jump_semitones=6):
    """Fix octave errors post-Viterbi."""
    ...
    for i in range(1, len(midi_vals)):
        ...
        # If jump is ~12 semitones (octave), might be error
...
# === Advanced Note Segmentation from Frames ===
def frames_to_notes_advanced(times, freqs, min_note_sec=MIN_NOTE_SEC):
    """Advanced note segmentation using pitch stability analysis."""
    if len(freqs) == 0:
        return []
    
    midi_vals = np.array([hz_to_midi(f) if f > 0 else 0 for f in freqs])
    
    # Identify stable pitch regions
    is_voiced = midi_vals > 0
    pitch_stable = np.ones(len(midi_vals), dtype=bool)
    
    for i in range(1, len(midi_vals) - 1):
        if is_voiced[i]:
            # Check if pitch is stable (within 0.4 semitones)
    ...
    for i in range(len(midi_vals)):
        if is_voiced[i] and pitch_stable[i]:
            if not in_note:
                ...
                # Continue note if pitch is similar (within 0.8 semitones)
                if abs(midi_vals[i] - np.median(note_pitches)) < 0.8:
                    note_pitches.append(midi_vals[i])
                else:
                    # Pitch changed - end note
            ...
            # Unvoiced or unstable - end current note
    ...
    # Merge nearby notes with similar pitch
    if not notes:
        return []
...
def notes_to_musicxml(notes_list, output_path, min_quarter_length=0.125):
    """Convert note list [(onset, offset, midi), ...] to MusicXML."""
    if not notes_list:
    ...
    for onset, offset, midi_val in notes_list:
        try:
            ...
            # Convert to quarter lengths with minimum threshold
            quarter_len = duration_sec * 2.0  # Assuming ~120 BPM
            
            # Skip notes that are too short for MusicXML
            ...
            print(f"Skipping note MIDI {midi_val}: {e}")
            skipped_count += 1
    
    if skipped_count > 0:
        print(f"Skipped {skipped_count} notes too short for MusicXML")
...
# === Harmony Generation ===
from music21 import converter, key as m21key, clef, meter, tempo
...
for root, _, files in os.walk(root_dir):
    ...
    for wav_file in wav_files:
        ...
        if not os.path.exists(json_path):
            print(f"Skipping {wav_path}, no matching JSON found")
            skipped += 1
            continue
        ...
        try:
            ...
            # Convert to notes using advanced segmentation
            ...
            # NEW: write harmony MusicXML
            write_harmonized_score_from_xml(pred_xml_path)

            # === Frame-level evaluation (raw) ===
            ref_time, ref_freq = json_to_pitch_sequence(json_path, sr=frame_rate)
            if ref_time is None:
                skipped += 1
                continue
            ...
            # === Frame-level evaluation (DTW-aligned) ===
...
print(f"\n Tip: Adjust CONFIDENCE_THRESHOLD ({CONFIDENCE_THRESHOLD}) for precision/recall tradeoff")
