"""
Pipeline C: pYIN + HMM/Viterbi pitch detection + JSON ground truth comparison.
"""
    ...
    for i, file_info in enumerate(files, 1):
        ...
        try:
            ...
            if not emotion_before:
                print("  Emotion classification failed, skipping")
                continue

            print(f"  Emotion: {emotion_before['top_emotion']} ({emotion_before['top_confidence']:.1%})")

            # Pitch detection with HMM
            times, smoothed_midi, f0, voiced_probs, y, sr = detect_pitch_pyin_hmm(str(wav_path))
